...
class Admin(commands.Cog):
	...
	def draft_id_required_error_message(self, draft_id):
		...
		"""
		if not draft_id:
			raise discord.ext.commands.errors.MissingRequiredArgument("draft id is" + self.draft_common_message)
	...
	def draft_ids_are_required_error_message(self, draft_id, draft_id2):
		...
		if not (draft_id and draft_id2):
			raise discord.ext.commands.errors.MissingRequiredArgument("draft ids are" + self.draft_common_message)
	...
	async def error_if_not_in_admin_channel(self, ctx: discord.ext.commands.Context):
		if ctx.message.channel not in self.allowed_channels:
			allowed_in = ", ".join([channel_id.mention for channel_id in self.allowed_channels])
			raise util.NotAdminChannel(f"Admin commands can only be executed in: {allowed_in}")
...
"""
	...
	@util.must_be_admin()
	async def draft(self, ctx, operation: str, draft_id: str = None, draft_id2: str = None):
		...
		if operation in ["create", "new", "c", "n"]:
			...
			self.bot.llama_firebase.create("vars", "drafts", draft_id2, self.bot.VARS["drafts"][draft_id2])
			...
			try:
				await ctx.send(
					...
					embed=discord.Embed.from_dict(json.loads(self.bot.VARS["drafts"][draft_id]))
				)
			except KeyError:
				await ctx.send(embed=discord.Embed(
			...
			await ctx.send(embed=discord.Embed(
				title=draft_id,
				description="```%s```" % json.dumps(json.loads(self.bot.VARS["drafts"][draft_id]), indent=4)
			))
		elif operation in ["list", "l"]:
			await ctx.send(embed=discord.Embed(
				title="List of drafts",
				description="Nothing found" if not self.bot.VARS["drafts"]
